...
def parse_date_fr(text):
    m = re.search(r"(\d{1,2})\.(\d{2})\.(\d{2,4})", text.strip())
    if m:
        d, mo, y = m.group(1), m.group(2), m.group(3)
        if len(y) == 2:
            y = "20" + y
        try:
            return datetime(int(y), int(mo), int(d), tzinfo=timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
        except ValueError:
            pass
...
def scrape_itconnect_article(url):
    ...
    if hero:
        ...
        if image_url.startswith("data:"):
            image_url = ""
    author = ""
    for sel in [".author a", ".entry-author a"]:
    ...
    if t:
        date_str = t.get("datetime", "")[:19].replace("T", " ")
    content_tag = soup.select_one("div.entry-content, div.post-content")
    content_html = clean_html(content_tag)
    excerpt = ""
    meta = soup.find("meta", {"name": "description"})
    if meta:
        excerpt = meta.get("content", "")
    tags = [a.get_text(strip=True) for a in soup.select(".tags a, .post-tags a")]
    return {
        "hash": make_hash(url),
        "cat_slug": "cyber",
        "source": "IT-Connect",
...
def scrape_incyber_article(url):
    ...
    content_tag = soup.select_one(".entry-content, .article-content, .post-content")
    if not content_tag:
    ...
    if meta:
        excerpt = meta.get("content", "")
    tags = [a.get_text(strip=True) for a in soup.select("a[href*='/categorie/']") if a.get_text(strip=True) not in ("Cyber +", "")]
...
def main():
    ...
    print("Done: " + str(len(unique)) + " articles")
